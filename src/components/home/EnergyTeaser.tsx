    ...
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: large ? "12px" : "4px", padding: large ? "20px" : "6px" }}>
      <div style={{ position: "relative", width: d + "px", height: d + "px" }}>
        {Array.from({ length: count }).map((_, i) => {
          ...
          return <div key={i} style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: size + "px", height: size + "px", border: "1px solid rgba(201,169,122,0.5)", borderRadius: "50%", opacity: 0.75 - i * 0.1 }} />;
        })}
      </div>
      <span style={{ color: GOLD, fontStyle: "italic", fontSize: large ? "13px" : "7px", letterSpacing: large ? "0.15em" : "0.08em", opacity: 0.85 }}>AdikAura</span>
      {large && <span style={{ color: GOLD, opacity: 0.5, fontSize: "10px", fontStyle: "italic" }}>אנרגיה יומית</span>}
    </div>
  );
}
...
function OracleCardFront({ card }: { card: (typeof ORACLE_CARDS)[0] }) {
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", padding: "18px 14px 14px", textAlign: "center" }}>
      <div style={{ color: GOLD, fontSize: "17px", marginBottom: "7px" }}>✦</div>
      <div style={{ color: GOLD, opacity: 0.65, fontSize: "10px", fontStyle: "italic", letterSpacing: "0.07em", marginBottom: "9px" }}>{card.series}</div>
      <h4 style={{ color: GOLD, fontSize: "13px", fontStyle: "italic", fontWeight: 600, lineHeight: 1.45, marginBottom: "9px" }}>{card.title}</h4>
      <div style={{ color: GOLD, opacity: 0.45, fontSize: "11px", letterSpacing: "0.14em", marginBottom: "9px" }}>✦ ✦ ✦</div>
      <p style={{ color: CREAM, fontSize: "11px", lineHeight: 1.85, fontWeight: 300, flex: 1, opacity: 0.88 }}>{card.msg}</p>
      <div style={{ color: GOLD, fontSize: "12px", marginTop: "8px", opacity: 0.45 }}>✦</div>
    </div>
  );
}
...
function InlineCardDraw() {
  ...
  return (
    <div>
      ...
      <p style={{ textAlign: "center", color: GOLD, fontSize: "13px", fontStyle: "italic", opacity: 0.85, marginBottom: "6px" }}>
        {isDone ? "קלף המסר שלך להיום ✦" : "בחרי קלף מסר להיום ✦"}
      </p>
      {!isDone && (
        <p style={{ textAlign: "center", color: CREAM, fontSize: "11px", opacity: 0.4, marginBottom: "16px" }}>
          לחצי על הקלף הגדול לבחירה אקראית — או עצרי את הקרוסלה ובחרי
        </p>
      )}
      ...
      <div style={{ display: "flex", justifyContent: "center", marginBottom: "18px" }}>
        <div
          ...
          onClick={() => { if (cardPhase === "choosing") pickCard(Math.floor(Math.random() * ORACLE_CARDS.length)); }}
        >
      ...
      {isDone && (
        <p style={{ textAlign: "center", color: GOLD, fontSize: "12px", opacity: 0.55, fontStyle: "italic", marginBottom: "14px", animation: "eFadeUp .5s ease" }}>
          המסר שלך להיום נבחר 🌿 חזרי מחר לקלף חדש
        </p>
      )}

      {!isDone && (
        <p style={{ textAlign: "center", color: GOLD, fontSize: "11px", opacity: 0.5, marginBottom: "8px" }}>
          {paused ? "✦ הקלפים עצרו — לחצי על אחד מהם" : "העבירי עכבר / גע כדי לעצור"}
        </p>
      )}
      ...
      <div
        ...
        onTouchEnd={() => setTimeout(() => setTouchPaused(false), 800)}
      >
          ...
          {[...ORACLE_CARDS, ...ORACLE_CARDS].map((_, i) => {
            ...
            return (
              <div
                ...
                style={{ width: "70px", height: "105px", flexShrink: 0, transform: `rotate(${offset * 1.6}deg)`, opacity: isDone ? (isSel ? 1 : 0.22) : 1, filter: isDone && !isSel ? "blur(1px) saturate(0.3)" : "none", outline: isSel ? "2px solid rgba(201,169,122,.65)" : "none", outlineOffset: "2px" }}
              >
      ...
      {toastVisible && (
        <div style={{ position: "fixed", bottom: "28px", left: "50%", transform: "translateX(-50%)", backgroundColor: "rgba(90,62,40,0.96)", border: "1px solid rgba(201,169,122,0.4)", color: CREAM, padding: "10px 24px", borderRadius: "40px", fontSize: "13px", animation: "eToast .3s ease", zIndex: 1000, whiteSpace: "nowrap" }}>
    ...
    <div className="w-full max-w-3xl mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {cards.map((card, i) => (
          ...
          >
            <div className="text-5xl font-bold mb-1" style={{ color: GOLD }}>{card.num}</div>
            <div className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: GOLD, opacity: 0.75 }}>{card.label}</div>
            <p className="text-xs font-bold mb-2 leading-snug" style={{ color: CREAM }}>{card.title}</p>
            {card.body && <p className="text-xs" style={{ color: CREAM, opacity: 0.58, lineHeight: 1.7 }}>{card.body}</p>}
          </motion.div>
        ))}
      </div>
    </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {view === "preview_form" && (
            <motion.div key="preview_form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.5 }} className="max-w-md mx-auto text-center">
              <p className="text-xs font-semibold uppercase tracking-widest mb-4" style={{ color: GOLD }}>גלי את האנרגיה שלך</p>
              <h2 className="text-3xl md:text-5xl font-bold mb-5 leading-snug" style={{ color: CREAM }}>
              ...
              <form onSubmit={handlePreviewSubmit} className="flex flex-col gap-4 text-right">
                ...
                <div>
                  <label className="block text-xs mb-1.5 text-right" style={{ color: GOLD }}>תאריך לידה</label>
                ...
                <button type="submit" className="w-full py-3.5 rounded-full font-bold text-sm transition-all duration-300 hover:shadow-md mt-2" style={{ backgroundColor: GOLD, color: "#5a3e28" }}>
                  גלי את האנרגיה שלך ✨
                </button>
              </form>
            </motion.div>
          ...
          {view === "preview_results" && nums && (
            <motion.div key="preview_results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.5 }}>
              <div className="text-center mb-8">
                <p className="text-xs font-semibold uppercase tracking-widest mb-2" style={{ color: GOLD }}>האנרגיה שלך היום</p>
                <h2 className="text-2xl md:text-3xl font-bold" style={{ color: CREAM }}>
              ...
              <div className="my-8 flex items-center gap-4 max-w-xl mx-auto">
                ...
                <span style={{ color: GOLD, opacity: 0.6, fontSize: "12px" }}>✦</span>
                <div className="flex-1 h-px" style={{ backgroundColor: "rgba(201,169,122,0.2)" }} />
              </div>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.3 }} className="text-center mt-4">
                <p className="text-base mb-5" style={{ color: CREAM, opacity: 0.82, lineHeight: 1.7 }}>
                  רוצה לדעת באופן קבוע את האנרגיה<br />היומית שלך ולקבל קלף מסר כל יום?
                </p>
                <button onClick={() => setView("register_form")} className="px-8 py-3.5 rounded-full font-bold text-sm transition-all duration-300 hover:shadow-md hover:-translate-y-0.5" style={{ backgroundColor: GOLD, color: "#5a3e28" }}>
                  כן! הירשמי בחינם ←
                </button>
              </motion.div>
            </motion.div>
          ...
          {view === "register_form" && (
            <motion.div key="register_form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.5 }} className="max-w-md mx-auto">
              <div className="text-center mb-8">
                ...
                <h2 className="text-2xl font-bold mb-2" style={{ color: GOLD }}>הירשמי לאזור האישי</h2>
                <p className="text-sm" style={{ color: CREAM, opacity: 0.65, lineHeight: 1.7 }}>גישה לאנרגיה ולקלף מסר שלך — כל יום</p>
              </div>
              <InlineRegister previewName={previewName} previewBirthDate={previewBirthDate} onSuccess={handleRegisterSuccess} />
              <button type="button" onClick={() => setView("preview_results")} className="block mx-auto mt-4 text-xs" style={{ color: CREAM, opacity: 0.35, background: "none", border: "none", cursor: "pointer" }}>
                חזרה
              </button>
            </motion.div>
          )}
          ...
          {view === "logged_in" && authUser && nums && (
            <motion.div key="logged_in" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.5 }}>
              <div className="text-center mb-8">
                <p className="text-xs font-semibold uppercase tracking-widest mb-2" style={{ color: GOLD }}>האנרגיה היומית שלך</p>
                <h2 className="text-2xl md:text-3xl font-bold" style={{ color: CREAM }}>שלום {authUser.name.split(" ")[0]} ✨</h2>
              </div>
              <EnergyCards nums={nums} />
              <div className="my-8 flex items-center gap-4 max-w-xl mx-auto">
                <div className="flex-1 h-px" style={{ backgroundColor: "rgba(201,169,122,0.2)" }} />
